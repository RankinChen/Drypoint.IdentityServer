using Drypoint.IdentityServer.Hosting.ViewModel;
...
namespace Drypoint.IdentityServer.Hosting.Controllers
{
    ...
    public class Is4ApiController : ControllerBase
    {
        ...
        public MessageModel<AccessApiDateView> GetAchieveUsers()
        {
            ...
            var users = _userManager.Users.Where(d => !d.IsDeleted).OrderByDescending(d => d.Id).ToList();

            var tadayRegisterUser = users.Where(d => d.Birthday >= DateTime.Now.Date).Count();

            apiDates = (from n in users
                        group n by new { n.Birthday.Date } into g
                        select new ApiDate
                        {
